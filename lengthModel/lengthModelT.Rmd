...
"
write(model, file = "~/mysis/stanModels/lengthT.stan")
# sm <- stan_model("~/mysis/stanModels/lengthT.stan")
sm <- stan("~/mysis/stanModels/lengthT.stan", 
...
estimate_params <- function(model, data, pars = pars, iter=1000){
...
#   pars = c("beta", "s"), iter = 100, chains = 1)
m <- estimate_params(sm, data = list(N=length(y), p=dim(X)[2], y=y, X=X),
                     pars = c("beta", "s", "gamma2", "lambda2", "nu"))
```
